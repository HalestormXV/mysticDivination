...
{
    @SubscribeEvent
    public void openChect(PlayerInteractEvent.RightClickBlock event)
    {
        if (!event.getWorld().isRemote)
        {
            ...
            if ((block instanceof BlockChest))
            {
                ...
                {
                    if (!tileTag.getString("divinationLock").equals(event.getEntityPlayer().getName()) && !event.getEntityPlayer().isCreative())
                    //if (!tileTag.getUniqueId("divinationLock").equals(event.getEntityPlayer().getPersistentID()))
                    {
    ...
    public void breakBlock(BlockEvent.BreakEvent event)
    {
        if (!event.getWorld().isRemote)
        {
            ...
            if ((block instanceof BlockChest))
            {
                TileEntity tileChest = event.getWorld().getTileEntity(event.getPos());
                NBTTagCompound tileTag = tileChest.getTileData();
                if ((tileTag.hasKey("divinationLock")) && (!tileTag.getString("divinationLock").equals(event.getPlayer().getName()) && !event.getPlayer().isCreative()))
                {
