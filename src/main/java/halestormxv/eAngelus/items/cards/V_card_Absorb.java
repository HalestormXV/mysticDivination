...
public class V_card_Absorb extends Item
{
    private int moralityRequirement = eAngelusConfig.moralityCost_AbsorbCard;

    public V_card_Absorb(String name)
    {
        super();
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn)
    {
        ...
        if (morality.getMorality() <= moralityRequirement)
        {
            if (playerIn.inventory.hasItemStack(new ItemStack(eAngelusItems.mystalDust)))
            {
                ...
                playerIn.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("absorption"), 1800, 4));
    ...
    {
        tooltip.add("");
        tooltip.add("\u00A76" + "The idea of wilderness");
        tooltip.add("\u00A76" + "needs no defense, it only");
        tooltip.add("\u00A76" + "needs defenders.");
        tooltip.add("");
